...
class b2c_finder_sales_rule_order{
    ...
    function detail_basic($id){
        ...
        //生效平台
        if($arr['apply_platform']) {
            $apply_platform = explode(',', $arr['apply_platform']);
            $platfromArr = array('1'=>'PC端','2'=>'触屏端');
            $platfrom = '';

            if($apply_platform) {
                foreach($apply_platform as $value){
                    $platform[] = $platfromArr[$value];
                }
            }
            $render->pagedata['platform'] = $platform;
        }
