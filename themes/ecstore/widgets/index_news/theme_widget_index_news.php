...
function article_new_show(&$smarty, $tmp, $setting, &$html, $lv=0, &$limit) {
    ...
    if($tmp['article']) {
        ...
        foreach ($tmp['article'] as $idx=>$row) {
            ...
            if($limit>0){
                if($idx == 0){
                    $html[$key] = "<li class='article-index-list'><ul class='article-list'>".article_new_html($tmp_lv, $url, $article_title,'article-index');
                }elseif($idx == ($len-1)){
                    $html[$key] = article_new_html($tmp_lv, $url, $article_title,'article-index')."</ul></li>";
                }else{
                    $html[$key] = article_new_html($tmp_lv, $url, $article_title,'article-index').'</li>';
                }
            }
