...
class b2c_cart_prefilter_promotion_goods implements b2c_interface_cart_prefilter {
    ...
    public function filter($aData,&$aResult,$aConfig) {
        ...
        if(!isset($aConfig['promotion']['goods'])) {//购物车的时候
            ...
            foreach($aResult['object']['goods'] as $row) {
                ...
                if($row['prepare_type'] && $row['prepare_type']=='prepare') return;
            ...
            $aConfig = $this->_init_rule(array_unique($aGoodsId),array('current_time'=>time(),'platform'=>$aData['apply_platform']));
        } else {
    ...
    private function _init_rule($aGoodsId,$filter = array()) {
        ...
        if( !$filter['platform'] ) $filter['platform'] = '-1';
...
//        echo $sSql;exit();
    ...
    private function _filter_sql($aFilter) {
        ...
        if (isset($aFilter['platform'])){
            $aWhere[] = ' (find_in_set(\''. $aFilter['platform'] .'\', sdb_b2c_goods_promotion_ref.apply_platform))';
    ...
    private function _filter_product(&$aData, &$aResult, &$aConfig) {
        ...
        foreach($aConfig as $key=>$rule) {
            ...
            if(is_array($action_solution)){
                $rule['s_template'] = key($action_solution); //优惠方案模板采用实际应用中的动作方案的key
            }
