...
class base_rpc_caller{
    ...
    public function call($method,$params,$rpc_id=null,$gzip=false){
        ...
        $query_params['to_node_type'] = $query_params['node_type'];

        if(PRISM_MODE == 'true'){
            $prism_path = array(
                    '1' => PRISM_MATRIX_ASYNC,
                    '2' => PRISM_MATRIX_SYNC,
                    '3' => PRISM_MATRIX_SERVICE,
                );
            $prism_client = new base_prism(PRISM_MATRIX_URL, PRISM_MATRIX_KEY, PRISM_MATRIX_SECRET);
            $prism_path_id = $this->network_id;
            $response = $prism_client->post($prism_path[$prism_path_id], $query_params, $headers);
        }else{
