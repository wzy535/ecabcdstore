...
class dbeav_filter{
    function dbeav_filter_parser($filter,$tableAlias=null,$baseWhere=null,&$object){
        ...
        if(isset($filter['point']) && !is_array($filter['point']) && !is_numeric($filter['point'])){
            unset($filter['point']);
        }
                ...
                if(isset($cols[$k])||strpos($k,'|')){
                    ...
                    if(is_array($v)){
                     ...
                     $ac = array();
                        ...
                        }elseif(!is_array($cols[$k]['type']) && (strpos($cols[$k]['type'],'int')!==false||$cols[$k]['type']=='money'||$cols[$k]['type']=='number'||$cols[$k]['type']=='float'||strpos($cols[$k]['type'],'decimal')!==false) && $filter['_'.$k.'_search']){
                           if($filter['_'.$k.'_search']=='between'){
